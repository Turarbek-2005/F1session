...
export default function StandingsScreen({ navigation }: any) {
  ...
  const [driversApi, setDriversApi] = useState<any>({
    drivers_championship: [],
  });
  const [teamsApi, setTeamsApi] = useState<any>({
    ...
    (async () => {
      try {
        const [d, t] = await Promise.all([
          f1ApiService.getStandingsDrivers
            ? f1ApiService.getStandingsDrivers()
            : {},
          f1ApiService.getStandingsTeams
            ? f1ApiService.getStandingsTeams()
            : {},
        ]);
    ...
    .map((driver: any) => {
      ...
      if (!stat) {
        return {
          ...
          matchedTeam: { teamImgUrl: "", teamId: "unknown" },
          position: Infinity,
        };
      }
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.container}>
          ...
          >
            <Text style={styles.driver}>
              {stat.driver?.shortName ||
                `${stat.driver?.name} ${stat.driver?.surname}`}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() =>
              navigation.navigate("TeamDetail", { teamId: matchedTeam?.id })
            }
          >
            <Text style={styles.team}>
              {stat.team?.teamName ?? matchedTeam?.teamName ?? stat.teamId}
            </Text>
          </TouchableOpacity>
      ...
      {sortedTeams.map(({ team, stat }: any) => (
        <View key={team.teamId} style={styles.row}>
          ...
          >
            <Text style={styles.driver}>
              {stat.team?.teamName ?? team.teamName}
            </Text>
          </TouchableOpacity>
